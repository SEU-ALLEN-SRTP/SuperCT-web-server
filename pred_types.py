...
def load_data(buffer, genes_used):
    data = pd.read_csv(buffer, header=0, index_col=0)
...
def plot_histogram(res):
    st.subheader('Cell type histogram')
    d = {}
...
def main():
    ...
    cell_types = load_cell_types('models/' + st.selectbox('Select a trained neural network model', ['v1_id2type.csv']))
    ...
    mapping_viewer = st.checkbox('View mapping results in tSNE', True)
    # input data
    st.header('Data')
    upload = st.file_uploader('Upload your DGE table', ['csv'],
                              help='differential gene expression table (csv), binary')
    if upload is not None:
        data = load_data(upload, genes_used)
        st.write('First 100 rows and columns of your uploaded table...')
        st.dataframe(data.iloc[0:99, 0:99])
        if st.button('Run Prediction'):
            ...
            st.dataframe(result_out.iloc[0:99])
            if plot_hist:
                plot_histogram(result_out)
            if mapping_viewer:
                plot_tsne(data, result_out)
            if mapping_viewer:
                st.subheader('Mapping Viewer')
